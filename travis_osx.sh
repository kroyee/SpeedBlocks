...
if [[ ! -d "$TGUI_ROOT/lib" ]]; then
  ...
  cmake -DCMAKE_INSTALL_PREFIX=$TGUI_ROOT -DCMAKE_INSTALL_FRAMEWORK_PREFIX=$TGUI_ROOT/lib -DTGUI_BUILD_FRAMEWORKS=TRUE -DSFML_INCLUDE_DIR=$SFML_ROOT/SFML.framework -DTGUI_OPTIMIZE_SINGLE_BUILD=TRUE .
  make -j2
  sudo make install
  cd ..
else
