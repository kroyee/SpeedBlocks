...
void OnlineplayUI::create(sf::Rect<int> _pos, UI* _ui) {
	createBase(_pos, _ui);

	opTab = ui->themeBB->load("Tab");
	opTab->add("Rooms");
	opTab->add("Lobby");
	...
	Lobby = ui->themeBB->load("ChatBox");
	Lobby->setPosition(5, 5);
	Lobby->setSize(750, 455);
	ServerLobby->add(Lobby);

	ChatBox = ui->themeTG->load("EditBox");
	ChatBox->setPosition(5, 465);
	ChatBox->setSize(750, 30);
	ChatBox->connect("ReturnKeyPressed", &UI::sendMsg, ui, "Lobby");
	ChatBox->connect("Focused Unfocused", &UI::chatFocus, ui, std::bind(&tgui::Widget::isFocused, ChatBox));
	ServerLobby->add(ChatBox);

	LobbyList = ui->themeBB->load("ListBox");
	...
	tournamentList.create(pos, ui, panel);
	...
	tgui::Label::Ptr NorL = ui->themeTG->load("Label");
	NorL->setPosition(30, 30);
	NorL->setText("Room name");
	CreateRoom->add(NorL);

	tgui::EditBox::Ptr NorE = ui->themeTG->load("EditBox");
	NorE->setPosition(25, 60);
	NorE->setSize(250, 40);
	NorE->connect("Focused Unfocused", &UI::chatFocus, ui, std::bind(&tgui::Widget::isFocused, NorE));
	CreateRoom->add(NorE);

	tgui::Label::Ptr NopL = ui->themeTG->load("Label");
	NopL->setPosition(430, 30);
	NopL->setText("Max players (0=unlimited)");
	CreateRoom->add(NopL);

	tgui::EditBox::Ptr NopE = ui->themeTG->load("EditBox");
	NopE->setPosition(425, 60);
	NopE->setSize(250, 40);
	NopE->setInputValidator(tgui::EditBox::Validator::UInt);
	NopE->connect("Focused Unfocused", &UI::chatFocus, ui, std::bind(&tgui::Widget::isFocused, NopE));
	CreateRoom->add(NopE);

	tgui::Button::Ptr CrB = ui->themeBB->load("Button");
...
void OnlineplayUI::createRoom(const sf::String& name, const sf::String& maxplayers) {
	...
	if (!maxplayers.getSize())
		return;
	ui->sendPacket11(name, stoi(maxplayers.toAnsiString()) );
	opTab->select(0);
}
...
void OnlineplayUI::addRoom() {
	...
	sf::Uint16 id;
	ui->net.packet >> id >> name >> currentPlayers >> maxPlayers;
...
void OnlineplayUI::makeClientList() {
	...
	ui->net.packet >> clientCount;

	for (int i=0; i<clientCount; i++) {
		ui->net.packet >> client.id >> client.name;
