...
void GameplayUI::create(sf::Rect<int> _pos, UI* _ui) {
	createBase(_pos, _ui);

	unknown = "Unknown";

	InGameTab = ui->themeBB->load("Tab");
	...
	tgui::Label::Ptr SbL = ui->themeTG->load("Label");
	SbL->setPosition(5, 5);
	...
	ChatBox->connect("ReturnKeyPressed", &UI::sendMsg, ui, std::bind(&tgui::Tab::getSelected, ChatTab));
...
void GameplayUI::scoreRow() {
	...
	ui->net.packet >> id >> combo >> sent >> received >> blocked >> bpm >> spm >> rank >> position >> score >> adj;
	...
	tgui::Label::Ptr label = ui->themeTG->load("Label");
