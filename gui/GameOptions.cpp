...
void GameOptions::create(sf::Rect<int> _pos, UI* _ui) {
	createBase(_pos, _ui);
	initSprites();

	otab = ui->themeBB->load("Tab"); // General Options
	otab->setTextSize(32);
	...
	tgui::Label::Ptr NL = ui->themeTG->load("Label");
	NL->setPosition(80, 13);
	NL->setText("Name");
	GenOpt->add(NL);
	tgui::EditBox::Ptr NTB = ui->themeTG->load("EditBox");
	NTB->setText(ui->options.name);
	NTB->setPosition(150, 10);
	NTB->setSize(400, 30);
	NTB->connect("TextChanged", &GameOptions::changeName, this);
	GenOpt->add(NTB);

	SelectKey = ui->themeTG->load("Panel");
	SelectKey->setSize(300, 100);
	SelectKey->setPosition(250, 150);
	SelectKey->hide();
	GenOpt->add(SelectKey);
	tgui::Label::Ptr CKPL = ui->themeTG->load("Label");
	CKPL->setTextSize(32);
	CKPL->setText("Press any key");
	CKPL->setPosition(40, 30);
	SelectKey->add(CKPL);

	tgui::Label::Ptr LeL = ui->themeTG->load("Label"); // Binding Keys
	LeL->setPosition(0, 63);
	LeL->setSize(90, 30);
	LeL->setText("Left");
	LeL->setHorizontalAlignment(tgui::Label::HorizontalAlignment::Right);
	GenOpt->add(LeL);
	BindLeft = ui->themeTG->load("Button");
	BindLeft->setPosition(100, 60);
	BindLeft->connect("pressed", &GameOptions::setKey, this, BindLeft, std::ref(ui->options.left));
	BindLeft->setText(SFKeyToString(ui->options.left));
	GenOpt->add(BindLeft);

	tgui::Label::Ptr RiL = ui->themeTG->load("Label");
	RiL->setPosition(0, 103);
	RiL->setSize(90, 30);
	RiL->setText("Right");
	RiL->setHorizontalAlignment(tgui::Label::HorizontalAlignment::Right);
	GenOpt->add(RiL);
	BindRight = ui->themeTG->load("Button");
	BindRight->setPosition(100, 100);
	BindRight->connect("pressed", &GameOptions::setKey, this, BindRight, std::ref(ui->options.right));
	BindRight->setText(SFKeyToString(ui->options.right));
	GenOpt->add(BindRight);

	tgui::Label::Ptr DoL = ui->themeTG->load("Label");
	DoL->setPosition(0, 143);
	DoL->setSize(90, 30);
	DoL->setText("Down");
	DoL->setHorizontalAlignment(tgui::Label::HorizontalAlignment::Right);
	GenOpt->add(DoL);
	BindDown = ui->themeTG->load("Button");
	BindDown->setPosition(100, 140);
	BindDown->connect("pressed", &GameOptions::setKey, this, BindDown, std::ref(ui->options.down));
	BindDown->setText(SFKeyToString(ui->options.down));
	GenOpt-> add(BindDown);

	tgui::Label::Ptr CwL = ui->themeTG->load("Label");
	CwL->setPosition(0, 223);
	CwL->setSize(90, 30);
	CwL->setText("Rotate >");
	CwL->setHorizontalAlignment(tgui::Label::HorizontalAlignment::Right);
	GenOpt->add(CwL);
	BindRCW = ui->themeTG->load("Button");
	BindRCW->setPosition(100, 220);
	BindRCW->connect("pressed", &GameOptions::setKey, this, BindRCW, std::ref(ui->options.rcw));
	BindRCW->setText(SFKeyToString(ui->options.rcw));
	GenOpt->add(BindRCW);

	tgui::Label::Ptr CcL = ui->themeTG->load("Label");
	CcL->setPosition(0, 183);
	CcL->setSize(90, 30);
	CcL->setText("Rotate <");
	CcL->setHorizontalAlignment(tgui::Label::HorizontalAlignment::Right);
	GenOpt->add(CcL);
	BindRCCW = ui->themeTG->load("Button");
	BindRCCW->setPosition(100, 180);
	BindRCCW->connect("pressed", &GameOptions::setKey, this, BindRCCW, std::ref(ui->options.rccw));
	BindRCCW->setText(SFKeyToString(ui->options.rccw));
	GenOpt->add(BindRCCW);

	tgui::Label::Ptr R1L = ui->themeTG->load("Label");
	R1L->setPosition(350, 63);
	R1L->setSize(140, 30);
	R1L->setText("Rotate 180");
	R1L->setHorizontalAlignment(tgui::Label::HorizontalAlignment::Right);
	GenOpt->add(R1L);
	BindR180 = ui->themeTG->load("Button");
	BindR180->setPosition(500, 60);
	BindR180->connect("pressed", &GameOptions::setKey, this, BindR180, std::ref(ui->options.r180));
	BindR180->setText(SFKeyToString(ui->options.r180));
	GenOpt->add(BindR180);

	tgui::Label::Ptr HdL = ui->themeTG->load("Label");
	HdL->setPosition(350, 103);
	HdL->setSize(140, 30);
	HdL->setText("Hard Drop");
	HdL->setHorizontalAlignment(tgui::Label::HorizontalAlignment::Right);
	GenOpt->add(HdL);
	BindHD = ui->themeTG->load("Button");
	BindHD->setPosition(500, 100);
	BindHD->connect("pressed", &GameOptions::setKey, this, BindHD, std::ref(ui->options.hd));
	BindHD->setText(SFKeyToString(ui->options.hd));
	GenOpt->add(BindHD);

	tgui::Label::Ptr ChL = ui->themeTG->load("Label");
	ChL->setPosition(400, 143);
	ChL->setSize(90, 30);
	ChL->setText("Chat");
	ChL->setHorizontalAlignment(tgui::Label::HorizontalAlignment::Right);
	GenOpt->add(ChL);
	BindChat = ui->themeTG->load("Button");
	BindChat->setPosition(500, 140);
	BindChat->connect("pressed", &GameOptions::setKey, this, BindChat, std::ref(ui->options.chat));
	BindChat->setText(SFKeyToString(ui->options.chat));
	GenOpt->add(BindChat);

	tgui::Label::Ptr ScL = ui->themeTG->load("Label");
	ScL->setPosition(400, 183);
	ScL->setSize(90, 30);
	ScL->setText("Score");
	ScL->setHorizontalAlignment(tgui::Label::HorizontalAlignment::Right);
	GenOpt->add(ScL);
	BindScore = ui->themeTG->load("Button");
	BindScore->setPosition(500, 180);
	BindScore->connect("pressed", &GameOptions::setKey, this, BindScore, std::ref(ui->options.score));
	BindScore->setText(SFKeyToString(ui->options.score));
	GenOpt->add(BindScore);

	tgui::Label::Ptr AwL = ui->themeTG->load("Label");
	AwL->setPosition(400, 223);
	AwL->setSize(90, 30);
	AwL->setText("Away");
	AwL->setHorizontalAlignment(tgui::Label::HorizontalAlignment::Right);
	GenOpt->add(AwL);
	BindAway = ui->themeTG->load("Button");
	BindAway->setPosition(500, 220);
	BindAway->connect("pressed", &GameOptions::setKey, this, BindAway, std::ref(ui->options.away));
	BindAway->setText(SFKeyToString(ui->options.away));
	GenOpt->add(BindAway);

	tgui::Button::Ptr Rp[7]; // Align Pieces
	tgui::Button::Ptr Cc[7];
	for (int i=0; i<7; i++) {
		Rp[i] = ui->themeBB->load("Button");
		Cc[i] = ui->themeBB->load("Button");
	...
	tgui::Label::Ptr Rl1 = ui->themeTG->load("Label"); // Repeat delay & speed
	Rl1->setPosition(750, 0);
	Rl1->setHorizontalAlignment(tgui::Label::HorizontalAlignment::Center);
	Rl1->setText("Left & Right Repeat\nDelay         Speed");
	GenOpt->add(Rl1);

	tgui::EditBox::Ptr Re1 = ui->themeTG->load("EditBox");
	Re1->setPosition(750, 50);
	Re1->setSize(70, 30);
	Re1->setInputValidator(tgui::EditBox::Validator::UInt);
	Re1->setText(to_string(ui->options.repeatDelay.asMilliseconds()));
	Re1->connect("TextChanged", &optionSet::setDelay, ui->options, 1);
	GenOpt->add(Re1);

	tgui::EditBox::Ptr Re2 = ui->themeTG->load("EditBox");
	Re2->setPosition(850, 50);
	Re2->setSize(70, 30);
	Re2->setInputValidator(tgui::EditBox::Validator::UInt);
	Re2->setText(to_string(ui->options.repeatSpeed.asMilliseconds()));
	Re2->connect("TextChanged", &optionSet::setDelay, ui->options, 2);
	GenOpt->add(Re2);

	tgui::Label::Ptr Rl2 = ui->themeTG->load("Label");
	Rl2->setPosition(750, 100);
	Rl2->setHorizontalAlignment(tgui::Label::HorizontalAlignment::Center);
	Rl2->setText("Down Repeat\n  Delay         Speed");
	GenOpt->add(Rl2);

	tgui::EditBox::Ptr Re3 = ui->themeTG->load("EditBox");
	Re3->setPosition(750, 150);
	Re3->setSize(70, 30);
	Re3->setInputValidator(tgui::EditBox::Validator::UInt);
	Re3->setText(to_string(ui->options.repeatDelayDown.asMilliseconds()));
	Re3->connect("TextChanged", &optionSet::setDelay, ui->options, 3);
	GenOpt->add(Re3);

	tgui::EditBox::Ptr Re4 = ui->themeTG->load("EditBox");
	Re4->setPosition(850, 150);
	Re4->setSize(70, 30);
	Re4->setInputValidator(tgui::EditBox::Validator::UInt);
	Re4->setText(to_string(ui->options.repeatSpeedDown.asMilliseconds()));
	Re4->connect("TextChanged", &optionSet::setDelay, ui->options, 4);
	GenOpt->add(Re4);
	...
	tgui::Label::Ptr ViL = ui->themeTG->load("Label");
	ViL->setPosition(350, 0);
	ViL->setText("Video Mode");
	VidOpt->add(ViL);

	VMSlider = ui->themeTG->load("Slider");
	VMSlider->setPosition(50, 50);
	VMSlider->setSize(700, 30);
	VMSlider->setMaximum(ui->options.modes.size()-1);
	if (ui->options.currentmode == -1)
		VMSlider->setValue(0);
	else
		VMSlider->setValue(ui->options.currentmode);
	VMSlider->connect("ValueChanged", &GameOptions::vidSlide, this);
	VMSlider->disable();
	VidOpt->add(VMSlider);

	VideoMode = ui->themeTG->load("Label");
	VideoMode->setPosition(340, 90);
	sf::String cvmname;
	cvmname = to_string(ui->options.modes[VMSlider->getValue()].width) + "x" + to_string(ui->options.modes[VMSlider->getValue()].height);
	VideoMode->setText(cvmname);
	VidOpt->add(VideoMode);

	Fullscreen = ui->themeTG->load("CheckBox");
	Fullscreen->setPosition(120, 130);
	Fullscreen->setText("Fullscreen");
	Fullscreen->connect("Checked Unchecked", &GameOptions::fsChecked, this);
	VidOpt->add(Fullscreen);
	if (ui->options.fullscreen) {
		Fullscreen->check();
		VMSlider->enable();
	}

	vSync = ui->themeTG->load("CheckBox");
	...
	tgui::Label::Ptr InL = ui->themeTG->load("Label");
	...
	tgui::CheckBox::Ptr EsC = ui->themeTG->load("CheckBox");
	...
	tgui::Slider::Ptr MvS = ui->themeTG->load("Slider");
	MvS->setPosition(50, 100);
	MvS->setSize(700,30);
	MvS->setMaximum(100);
	MvS->connect("ValueChanged", &GameOptions::volSlide, this, 1);
	MvS->setValue(ui->options.MusicVolume);
	SndOpt->add(MvS);

	tgui::Label::Ptr MvL = ui->themeTG->load("Label");
	MvL->setText("Music Volume");
	MvL->setPosition(350, 50);
	SndOpt->add(MvL);

	tgui::Slider::Ptr EvS = ui->themeTG->load("Slider");
	EvS->setPosition(50, 200);
	EvS->setSize(700,30);
	EvS->setMaximum(100);
	EvS->connect("ValueChanged", &GameOptions::volSlide, this, 2);
	EvS->setValue(ui->options.EffectVolume);
	SndOpt->add(EvS);

	tgui::Label::Ptr EvL = ui->themeTG->load("Label");
	EvL->setText("Effect Volume");
	EvL->setPosition(350, 150);
	SndOpt->add(EvL);

	tgui::Slider::Ptr CvS = ui->themeTG->load("Slider");
	CvS->setPosition(50, 300);
	CvS->setSize(700,30);
	CvS->setMaximum(100);
	CvS->connect("ValueChanged", &GameOptions::volSlide, this, 3);
	CvS->setValue(ui->options.ChatVolume);
	SndOpt->add(CvS);

	tgui::Label::Ptr CvL = ui->themeTG->load("Label");
...
void GameOptions::changeName(const sf::String& name) {
	ui->options.name = name;
	ui->game.field.setName(name);
}

void GameOptions::vidSlide(short i) {
	sf::String name;
	name = to_string(ui->options.modes[i].width) + "x" + to_string(ui->options.modes[i].height);
...
void GameOptions::applyVideo() {
	if (Fullscreen->isChecked()) {
		if (ui->options.currentmode == -1 || ui->options.currentmode != VMSlider->getValue()) {
			ui->options.currentmode = VMSlider->getValue();
			ui->window->close();
			ui->window->create(ui->options.modes[ui->options.currentmode], "SpeedBlocks", sf::Style::Fullscreen);
			ui->window->setView(sf::View(sf::FloatRect(0, 0, 960, 600)));
			ui->options.fullscreen=true;
		}
	}
	else if (ui->options.currentmode != -1) {
		ui->window->close();
		ui->window->create(sf::VideoMode(960, 600), "SpeedBlocks");
		ui->options.currentmode = -1;
		ui->window->setView(sf::View(sf::FloatRect(0, 0, 960, 600)));
		ui->options.fullscreen=false;
	}

	if (vSync->isChecked()) {
		ui->options.vSync = true;
		ui->window->setVerticalSyncEnabled(true);
	}
	else {
		ui->options.vSync = false;
		ui->window->setVerticalSyncEnabled(false);
	}

	if (performanceOutput->isChecked()) {
		ui->options.performanceOutput = true;
		ui->performanceOutput.show();
	}
	else {
		ui->options.performanceOutput = false;
		ui->performanceOutput.hide();
	}
	...
	if (fd.size())
		value = stoi(fd);
	if (value)
		ui->options.frameDelay = sf::microseconds(1000000/value);
	value=0;
	fd = InputDelay->getText();
	if (fd.size())
		value = stoi(fd);
	if (value)
		ui->options.inputDelay = sf::microseconds(1000000/value);
}

void GameOptions::volSlide(short i, short vol) {
	if (i == 1) {
		ui->options.MusicVolume = vol;
		ui->resources.sounds.setMusicVolume(vol);
	}
	else if (i == 2) {
		ui->options.EffectVolume = vol;
		ui->resources.sounds.setEffectVolume(vol);
	}
	else if (i == 3) {
		ui->options.ChatVolume = vol;
		ui->resources.sounds.setChatVolume(vol);
	}
}
...
void GameOptions::putKey(sf::Event& event) {
	if (event.type == sf::Event::KeyPressed) {
        if (event.key.code == sf::Keyboard::Key::Escape || event.key.code == -1 ) {
			...
			if (event.key.code == ui->options.left) {
				ui->options.left = sf::Keyboard::Unknown;
				BindLeft->setText("");
			}

			if (event.key.code == ui->options.right) {
				ui->options.right = sf::Keyboard::Unknown;
				BindRight->setText("");
			}

			if (event.key.code == ui->options.down) {
				ui->options.down = sf::Keyboard::Unknown;
				BindDown->setText("");
			}

			if (event.key.code == ui->options.rcw) {
				ui->options.rcw = sf::Keyboard::Unknown;
				BindRCW->setText("");
			}

			if (event.key.code == ui->options.rccw) {
				ui->options.rccw = sf::Keyboard::Unknown;
				BindRCCW->setText("");
			}

			if (event.key.code == ui->options.r180) {
				ui->options.r180 = sf::Keyboard::Unknown;
				BindR180->setText("");
			}

			if (event.key.code == ui->options.hd) {
				ui->options.hd = sf::Keyboard::Unknown;
				BindHD->setText("");
			}

			if (event.key.code == ui->options.chat) {
				ui->options.chat = sf::Keyboard::Unknown;
				BindChat->setText("");
			}

			if (event.key.code == ui->options.score) {
				ui->options.score = sf::Keyboard::Unknown;
				BindScore->setText("");
			}

			if (event.key.code == ui->options.away) {
				ui->options.away = sf::Keyboard::Unknown;
				BindAway->setText("");
			}
...
void GameOptions::rotPiece(short i) {
	ui->options.piecerotation[i]++;
	if (ui->options.piecerotation[i]>3)
		ui->options.piecerotation[i]=0;
	piece[i].setRotation(ui->options.piecerotation[i]*90);
	ui->game.updateBasePieces();
}

void GameOptions::colPiece(short i) {
	if (ui->options.basepiece[i].tile+1>7)
        ui->options.setPieceColor(i, 1);
    else
        ui->options.setPieceColor(i, ui->options.basepiece[i].tile+1);
	piece[i].setColor(pColor(ui->options.basepiece[i].tile));
	ui->game.updateBasePieces();
}
...
void GameOptions::initSprites() {
	...
	std::vector<short> value = ui->options.pieceArray();
	...
	for (int p=0; p<7; p++) {
		...
		if (p==4 || p==6) {
			...
			piece[p].setColor(pColor(ui->options.basepiece[p].tile));
			piece[p].setOrigin(60,60);
			piece[p].setRotation(ui->options.piecerotation[p]*90);
		}
		else {
			for (int x=0; x<3; x++)
				for (int y=0; y<3; y++)
