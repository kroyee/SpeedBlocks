...
MovingPoint::MovingPoint(sf::Vector2f start) {
    ...
    for (int i = 0; i < 5; i++) pos.push_back(start);
    turnVal = 1;
    direction = rand() % 4;
    distance = 0;
}
...
AnimatedBackground::AnimatedBackground(Resources& resources, uint8_t count)
    ...
    for (int i = 0; i < count; i++) points.push_back(MovingPoint(sf::Vector2f(i * 960 / count, i * 600 / count)));
    ...
    for (auto& point : points) {
        ...
        while (point.distance > 30) {
            ...
            if (turn == 2) {
                ...
                point.turnVal--;
                point.direction--;
                if (point.direction > 3) point.direction = 3;
            } else {
                point.turnVal++;
                point.direction++;
                if (point.direction > 3) point.direction = 0;
            }
...
void AnimatedBackground::dark() {
    ...
    color = dark_color;
    for (auto& point : points) point.color = color;
}
