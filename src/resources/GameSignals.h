...
class Signal<void, Args...> {
   ...
   public:
    ...
    void operator()(Args2 &&... args2) {
        ...
        for (auto &func : function) func(std::forward<Args2>(args2)...);
    }
    ...
    void connect(void (Class::*func)(Args...), Instance instance) {
        function.emplace_back([instance, func](Args &&... args) { (instance->*func)(std::forward<Args>(args)...); });
    }

    void disconnect() { function.clear(); }

    static Signal &get(std::string signal_name) {
        static std::unordered_map<std::string, Signal> signal_map;

        if (signal_map.find(signal_name) == signal_map.end()) signal_map[signal_name] = Signal();
...
class Signal<ReturnType, Args...> {
   ...
   public:
    ...
    void connect(ReturnType (Class::*func)(Args...), Instance instance) {
        function = [instance, func](Args &&... args) -> ReturnType { return (instance->*func)(std::forward<Args>(args)...); };
    }
...
class Net {
   ...
   public:
    ...
    static void takePacket(std::size_t id, const Func &func) {
        if (id >= packetReceivers.size()) packetReceivers.resize(id + 1);
    ...
    static void takeSignal(std::size_t id, const Func &func) {
        if (id >= signalReceivers.size()) signalReceivers.resize(id + 1);
    ...
    static bool passOnSignal(std::size_t id, Args... args) {
        if (id < signalReceivers.size()) {
            SignalHolder &ref = *signalReceivers[id];
            if (ref.size != sizeof...(Args)) return false;
