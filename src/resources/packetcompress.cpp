...
void PacketCompress::getBits(uint8_t& byte, uint8_t bits) {
    ...
    if (bitcount > 7) {
        bitcount -= 8;
        tmpcount++;
        if (bitcount > 0) temp = tmp[tmpcount] << (bits - bitcount) | temp;
    }
    temp = temp << (8 - bits);
    temp = temp >> (8 - bits);
    byte = temp;
...
void PacketCompress::addBits(uint8_t byte, uint8_t bits) {
    if (tmpcount >= tmp.size()) tmp.push_back(0);
...
void PacketCompress::clear() { tmp.clear(); }
...
void PacketCompress::compressReplay(Recording& replay, std::vector<uint8_t>& data) {
    clear();
    tmpcount = 0;
    bitcount = 0;
    uint8_t counter = 0;
...
void PacketCompress::extractReplay(Recording& replay, const NP_Replay& p) {
    replay.clear();
    clear();
    tmpcount = 100;
    bitcount = 0;
