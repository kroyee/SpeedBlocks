...
PatchCheck::PatchCheck() { connectSignal("ApplyPatch", &PatchCheck::apply, this); }
...
int PatchCheck::apply() {
    ...
    for (const auto file : j1) {
        ...
        if (applyAsAdmin) system("move /y SpeedBlocks.exe.old SpeedBlocks.exe");
#elif __APPLE__
