...
GPBaseState::GPBaseState(GamePlay& _game, GameStates _state) : state(_state), game(_game) { game.resources.gamestate = state; }
...
void GPBaseState::handleGeneralButtons(sf::Event& event) {  // != Replay && MainMenu
    ...
    if (event.type == sf::Event::KeyPressed) {
        ...
        if (event.key.code == score) Hide(13);
}
...
GPMainMenu::~GPMainMenu() { game.showPressEnterText = true; }
...
void GPCountDown::update() {
    if (!game.resources.playonline)
        if (game.countDown()) SetGameState(GameStates::Game);
}
...
void GPGame::update() { game.delayCheck(); }
...
GPGameOver::GPGameOver(GamePlay& _game) : GPBaseState(_game, GameStates::GameOver) {
    if (game.autoaway && game.resources.playonline) SetAway(true);
    game.field.text.hide<FieldText::Countdown>();
    game.drawMe = true;
}
...
void GPReplay::update() {
    if (game.playReplay()) SetGameState(GameStates::GameOver);
}
...
void GPPractice::update() { game.delayCheck(); }
