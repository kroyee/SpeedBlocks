...
TextObject& TextObject::size(uint8_t text_size, uint8_t header_size) {
    ...
    if (header_size) headerText.setCharacterSize(header_size);
...
void TextObject::draw(sf::RenderTexture& texture) {
    if (visible) {
        ...
        if (useHeader) texture.draw(headerText);
    }
}

void TextObject::update_origin() {
    auto text_origin = text.getLocalBounds().width / 2;
    if (std::abs(text.getOrigin().x - text_origin) > 5) text.setOrigin(text.getLocalBounds().width / 2, 0);

    auto header_origin = headerText.getLocalBounds().width / 2;
    if (std::abs(headerText.getOrigin().x - header_origin) > 5) headerText.setOrigin(headerText.getLocalBounds().width / 2, 0);
}
...
void initTextObjects(std::vector<TextObject>& vec, const std::vector<TextObjectInitializer>& data) {
    ...
    for (unsigned i = 0; i < vec.size(); ++i) {
        ...
        if (!data[i].header.empty()) textObject.header(data[i].header);

        if (data[i].hide) textObject.hide();
    }
}
...
GameFieldText::GameFieldText(Resources& _resources, sf::RenderTexture& tex) : resources(_resources), texture(tex) {
    ...
    initTextObjects(text, {{FieldText::Name, 32, {150, 555}, "Player", "", false},
...
void GameFieldText::setFont(const sf::Font& font) {
    for (auto& t : text) t.setFont(font);
}
...
bool GameFieldText::setComboTimer(uint8_t count) {
    ...
    if (comboTimer.getPointCount() == static_cast<unsigned int>(count + 2)) return false;
    comboTimer.setPointCount(count + 2);

    comboTimer.setPoint(0, sf::Vector2f(60, 60));
    for (int x = 1; x < (count + 2); x++) comboTimer.setPoint(x, sf::Vector2f(60 + 60 * cos((PI * 2) / 100 * (x - 26)), 60 + 60 * sin((PI * 2) / 100 * (x - 26))));
...
void GameFieldText::drawText() {
    ...
    for (auto& t : text) t.draw(texture);
}
