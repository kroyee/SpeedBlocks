...
int main() {
    ...
    if (!resources.init()) return 0;

    GamePlay game(resources);

#ifndef DEBUG
    if (Options::get<bool>("fullscreen")) window.create(Options::get_videomodes()[Options::get<short>("currentmode")], "SpeedBlocks", sf::Style::Fullscreen);
    if (!window.isOpen()) {
    ...
    if (Options::get<bool>("vsync")) window.setVerticalSyncEnabled(true);
...

    {
        ...
        while (intro) {
            ...
            while (window.pollEvent(event)) {
                if (event.type == sf::Event::KeyPressed) intro = false;
                if (event.type == sf::Event::MouseButtonPressed) intro = false;
    ...
    game.field.status = 5;
    gameDraw.quit();
    if (game.field.drawThread.joinable()) game.field.drawThread.join();
