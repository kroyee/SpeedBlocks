...
namespace tgui {
  namespace SBGui {
    ...
    MAKE_CHECK(setHorizontalAlignment, Label::HorizontalAlignment::Center)

    template <Label::HorizontalAlignment A>
    ...
    template <typename W>
    using is_widget_t = typename std::enable_if<std::is_base_of<Widget, W>::value, int>::type;
    ...
    template <typename... T>
    struct Align {
      Align(Panel::Ptr& base, int x, int y, int gap=0, bool down = true) : base(base.get()), x(x), y(y), gap(gap), step(0), down(down) {}
      ...
      Align(W& w, int gap = 0, bool down = true) {
        base = static_cast<Panel*>(w->getParent());
        if (down) {
          x = w->getPosition().x;
