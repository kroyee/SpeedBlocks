...
ChallengesGameUI::ChallengesGameUI(sf::Rect<int> _pos, Resources& _res) : GuiBase(_pos, _res), challenge(nullptr) {
    header.pos(450, 50).size(470, 70).text_size(60).add_to(panel).center();

    for (int i = 0; i < LabelCount; ++i) {
        info_fields[i].pos(660, 87 + i * 60).size(200, 40).hide().add_to(panel);
    }

    startChallenge.pos(550, 510).size(200, 40).text("Start challenge").connect("pressed", [&]() { Ready(); }).add_to(panel);

    special.pos(330, 230).size(90, 40).title_top().add_to(panel);
    ...
    connectSignal("Survivor", [&]() -> bool {
        if (panel->isVisible() && challenge->type == Challenges::Survivor) return true;
        return false;
    });
    connectSignal("Cheese30L", [&]() -> bool {
        if (panel->isVisible() && challenge->type == Challenges::Cheese30L) return true;
        return false;
    });
}

void ChallengesGameUI::clear() {
    if (challenge != nullptr) challenge->clear();
}
...
void ChallengesGameUI::hideStartChallengeButton() { startChallenge.hide(); }
...
void BaseChallenge::setLabel(int i, const std::string& text) {
    if (i >= LabelCount) return;
    ref.info_fields[i].show().title(text);
}

void BaseChallenge::setSpec(int i) {
    if (i >= LabelCount) return;
    ref.special.show().title(ref.info_fields[i]->getText());
    specIndex = i;
    if (i == 0)
        ref.special.text_size(18);
    else
        ref.special.text_size(27);
}
void BaseChallenge::updateSpec() {
    if (specIndex == LabelCount) return;
    ref.special.text(ref.info_fields[specIndex]->getText());
}
void BaseChallenge::setTime() {
    if (ref.resources.gamestate == GameStates::Replay)
        ref.info_fields[0].text(ref.displayTime(GetRecTime()));
    else
        ref.info_fields[0].text(ref.displayTime(GetGameTime()));
}
void BaseChallenge::clear() {
    for (auto& field : ref.info_fields) {
        field.text("");
    }
    ref.special.text("");
}
...
void CH_Race::update(GameplayData& data) {
    ref.info_fields[1].text(to_string(40 - data.linesCleared));
    ref.info_fields[2].text(to_string(data.pieceCount));

    updateSpec();

    if (data.linesCleared > 39) GameOver(1);
}
...
void CH_Cheese::update(GameplayData& data) {
    ref.info_fields[1].text(to_string(9 - data.garbageCleared));
    ref.info_fields[2].text(to_string(data.pieceCount));

    updateSpec();

    if (data.garbageCleared > 8) GameOver(1);
}
...
CH_Survivor::~CH_Survivor() { GameAddDelay(450); }

void CH_Survivor::update(GameplayData& data) {
    ref.info_fields[1].text(to_string(data.garbageCleared));
    ref.info_fields[2].text(to_string(data.pieceCount));
...
void CH_Survivor::clear() {
    for (int i = 0; i < LabelCount; i++) ref.info_fields[i].text("");
    ref.special.text("");
...
void CH_Cheese30L::update(GameplayData& data) {
    ref.info_fields[1].text(to_string(30 - data.garbageCleared));
    ref.info_fields[2].text(to_string(data.pieceCount));

    if (ref.resources.gamestate == GameStates::Game) {
        if (data.garbageCleared > 29) GameOver(1);
