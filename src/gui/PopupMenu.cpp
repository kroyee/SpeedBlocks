...
void PopupMenuItem::createLabel(bool _submenu, const std::string& text) {
    ...
    if (!submenu) {
        ...
        label->connect("MouseLeft", [&]() {
            if (!menu->mouseOver(menu->panel.get())) menu->hide();
        });
        label->setText(text + " >");
    }
}
...
PopupMenu::PopupMenu(Resources& res) : GuiBase({0, 0, 0, 0}, res), parent(nullptr) {
    ...
    panel->getRenderer()->setBackgroundColor(sf::Color(128, 128, 128, 220));
...
PopupMenu* PopupMenu::get(const std::string& text) {
    for (auto& item : items)
        if (item.submenu && item.label->getText() == text + " >") return item.menu.get();
    return nullptr;
}
...
void PopupMenu::update() {
    ...
    for (auto& item : items) {
        if (item.label->getSize().x > width) {
            ...
            if (item.submenu) item.menu->update();
        }
    }
    height = 0;
    for (auto& item : items) {
...
void PopupMenu::hideMe() {
    for (auto& item : items)
        if (item.submenu)
            if (mouseOver(item.menu->panel.get())) return;
    hide();
}

void PopupMenu::hide() {
    panel.hide();
    if (parent != nullptr) parent->childClosed();
}
...
void PopupMenu::show(int x, int y) {
    if (parent) {
        ...
        x = coords.x;
        y = coords.y;
        if (x < stayInside.left) x = stayInside.left;
        if (x + width > stayInside.left + stayInside.width) x = stayInside.left + stayInside.width - width;
    }
    if (y < stayInside.top) y = stayInside.top;
    if (y + height > stayInside.top + stayInside.height) y = stayInside.top + stayInside.height - height;
...
void PopupMenu::setBoundery(const sf::Rect<int>& _stayInside) {
    ...
    for (auto& item : items)
        if (item.submenu) item.menu->setBoundery(stayInside);
}

void PopupMenu::showIfInside(const sf::Vector2i& pos) {
    if (stayInside.contains(pos)) show(pos.x, pos.y);
}
